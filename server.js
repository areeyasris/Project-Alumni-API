...
app.post('/create', (req, res) => {
    ...
    db.query('INSERT INTO alumni (card_id, email, fname, midname, lname, faculty, major, telnum, address_line1, locality, district, province, postcode, certificates, statuscareer, career, year, salary) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
        [card_id, email, fname, midname, lname, faculty, school, tel, address, locality, district, province, postcode, certificates, statuscareer, career, year, salary], (err, result) => {
            if (err) {
                console.log(err)
            } else {
                res.send(result)
...
app.put("/update", (req, res) => {
    ...
    db.query(
        "UPDATE alumni SET card_id = ?, email = ? , fname = ?,midname = ?,lname = ? ,faculty = ?,major = ?,telnum = ?,address_line1 = ?,locality = ?,district = ?,province = ?,postcode = ? ,certificates = ? ,statuscareer = ? ,career = ? ,year = ? ,salary = ? WHERE alumni_id = ?",
        [cardid, email, fname, midname, lname, faculty, school, tel, address, locality, district, province, postcode, certificates, statuscareer, career, year, salary, id],
        (err, result) => {
            if (err) {
                console.log(err);
                return
...
app.get("/exportalumni", async (req, res) => {
    ...
    var sql = 'SELECT * FROM alumni'
    if (search) {
        sql += ' WHERE CONCAT( fname, lname, certificates, statuscareer, career) LIKE ?'
        params.push('%' + search + '%')
    }
    if (sort_column) {
        sql += ' ORDER BY ' + sort_column + ' ' + sort_direction;
